...
import { ArrowRight, MapPinned, ShieldAlert, Zap } from "lucide-react";
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-slate-950 px-6 py-20 text-slate-100 flex flex-col justify-between">
      ...
      <motion.main
        ...
        initial="hidden"
        animate="visible"
        className="my-auto grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto w-full z-10 pt-10 pb-10"
      >
        {/* 기존 동플 (클래식) 카드 */}
        ...
        <motion.div variants={itemVariants}>
          <Link
            href="/admin"
            className="group block h-full relative overflow-hidden rounded-[32px] border border-slate-800/80 bg-slate-900/40 p-8 shadow-xl backdrop-blur-md transition-colors hover:border-indigo-500/30 hover:bg-slate-900/60"
          >
            {/* 은은한 내부 조명 */}
            ...
            <h2 className="text-2xl font-black text-slate-100 flex items-center gap-2">
              동플 관리자
              <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-indigo-500/10 text-indigo-400 border border-indigo-500/10">Admin</span>
            </h2>

            <p className="mt-3 text-[14px] font-semibold leading-relaxed text-slate-400 group-hover:text-slate-300 transition-colors">
              제보 및 현황 검수, 사용자 인증 정보 확인, 신뢰성 지표 분석 및 동네 생활 시스템 전반의 운영 설정을 점검하는 관리자 전용 대시보드 모드입니다.
            </p>

            <div className="mt-8 flex items-center justify-between text-indigo-400 text-[13px] font-black pt-4 border-t border-slate-800/40 group-hover:border-indigo-500/20 transition-colors">
              <span>관리 대시보드 바로가기</span>
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-500/10 group-hover:translate-x-1.5 transition-transform">
                <ArrowRight size={14} />
              </div>
            </div>
          </Link>
