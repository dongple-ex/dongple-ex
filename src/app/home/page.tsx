...
export default function Home() {
  ...
  // 내 주변 2km 이내에 실시간 제보가 들어왔는지 여부 판정
  const isReportNearby = useMemo(() => {
    if (!latestActiveReport || !latestActiveReport.latitude || !latestActiveReport.longitude) return false;
    const dLat = (latestActiveReport.latitude - userLat) * (Math.PI / 180);
    const dLon = (latestActiveReport.longitude - userLng) * (Math.PI / 180);
    const a =
      Math.sin(dLat / 2) * Math.sin(dLat / 2) +
      Math.cos(userLat * (Math.PI / 180)) *
      Math.cos(latestActiveReport.latitude * (Math.PI / 180)) *
      Math.sin(dLon / 2) *
      Math.sin(dLon / 2);
    const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
    const dist = 6371 * c; // Distance in km
    return dist <= 2.0; // 2km 이내
  }, [latestActiveReport, userLat, userLng]);
    ...
    <div className="min-h-screen bg-background pb-32 text-foreground">
      ...
      {loading ? (
        ...
        <div className="mx-auto mt-3 grid max-w-md grid-cols-1 gap-4 px-5 md:max-w-[1120px] md:grid-cols-2 md:px-6">
          ...
          {latestActiveReport ? (
            <article className={`w-full overflow-hidden rounded-[22px] border bg-card-bg shadow-sm transition-all hover:shadow-md ${
              isReportNearby 
                ? "border-emerald-500/40 shadow-[0_0_20px_rgba(16,185,129,0.12)] ring-1 ring-emerald-500/20" 
                : "border-border"
            }`}>
              <div
                className="relative h-32 bg-cover bg-center"
              ...
              >
                <div className="absolute inset-0 bg-black/35" />
                <div className="absolute left-3.5 top-3.5 flex flex-wrap gap-2 items-center">
                  ...
                  {isReportNearby && (
                    <motion.span
                      ...
                      className="inline-flex items-center rounded-full bg-emerald-500 px-3 py-1.5 text-[11px] font-black text-white shadow-md gap-1"
                    >
                      <MapPinned size={11} className="animate-bounce" />
                      내 주변 정보
                    </motion.span>
                  )}
                </div>
