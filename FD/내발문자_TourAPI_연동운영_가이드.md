# 내발문자 TourAPI 연동 운영 가이드

## 목적
- 한국관광공사 OpenAPI를 내발문자의 공식 관광 데이터 축으로 운영한다.
- 내발문자 자체 데이터와 공공데이터를 분리해서 관리한다.
- 어떤 화면이 어떤 API를 사용하는지 FD에서 추적 가능하게 유지한다.
- TourAPI 행사 정보는 `행사 일정 앱`을 만들기 위한 것이 아니라, 행사 현장의 현재 상태 공유를 만들기 위한 진입점으로 사용한다.
- 핵심 구조는 `공공데이터 = 뼈대`, `내발문자 = 실시간 뇌`다.
...
- 행사 레이어는 초기 트래픽을 만들고, 관광지 주변 카페/식당/공원 같은 방문 맥락 장소 레이어는 평소 사용 빈도를 만든다.
...
## 다음 구현 체크리스트
- [ ] 홈에 `지금 핫한 곳` 섹션 구현
- [ ] 지도 필터를 `전체 / 행사 / 주변 카페 / 식당 / 공원` 중심으로 재정렬
- [ ] 행사 마커에 현재 상태 뱃지 연결
